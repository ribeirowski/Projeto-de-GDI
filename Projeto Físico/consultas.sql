...
SELECT P.NOME, (SELECT COUNT(*) FROM PET WHERE CPF_DONO = P.CPF) AS QTD_PETS, 
                (SELECT COUNT(*) FROM TERRITORIO WHERE CPF_CAPITAO = P.CPF) AS QTD_TERRITORIOS
FROM PIRATA P 
WHERE P.CPF = (SELECT C.CPF FROM CAPITAO C WHERE C.CPF = '003');
